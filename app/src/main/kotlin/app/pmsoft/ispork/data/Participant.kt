...
open class Participant(
  ...
  @Ignore
  constructor(type: Type, defaultCurrency: Currency) : this(
    type,
    0,
    "",
    true,
...
class FullParticipant(
  ...
  constructor(participant: Participant, moneyBags: List<FullMoneyBag>) : this(
    ...
    participant.defaultCurrency,
    participant.bookingDateIsEntryDate,
    moneyBags
  )

  fun getBalanceFor(currency: Currency): Amount {
    return moneyBags.firstOrNull { it.currency == currency }
      ?.let { it.bookedBalance + it.startingBalance } ?: 0L
  }
}
